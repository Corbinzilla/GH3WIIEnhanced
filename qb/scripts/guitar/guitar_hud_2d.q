...
script create_hot_start
	...
	Wait 0.5 seconds
	doScreenElementMorph id = hot_start_text alpha = 0 time = 0.5
	Wait 0.5 seconds
	DestroyScreenElement \{id = hot_start_text}
endscript
...
script create_2d_hud_elements \{player_text = 'p1'}
	...
	if NOT ($game_mode = p2_battle || $boss_battle = 1)
		...
		score_text_pos = (222.0, 80.0)
		if ($game_mode = p2_career || $game_mode = p2_coop)
			<score_text_pos> = (226.0, 95.0)
		endif
		...
		if ScreenElementExists id = <new_note_container>
			...
			displayText {
				...
				font = num_a7
				text = '0'
				pos = ((222.0, 70.0) + (<i> * (-37.0, 0.0)))
				z = 25
				just = [center center]
				rgba = <rgba>
				noshadow
			}
			<id> :SetTags intial_pos = ((222.0, 70.0) + (<i> * (-37.0, 0.0)))
		endif
		<i> = (<i> + 1)
		repeat 4
	endif
