...
script is_in_net_lobby 
	if ($is_network_game = 1)
		return \{open = $in_net_lobby}
	endif
	return \{0}
endscript
...
script create_character_select_menu \{player = 1}
	...
	purpley = [189 126 255 80]
	orangey = [189 126 255 80]
	purpley_spotlight = [189 126 255 80]
	orangey_spotlight = [189 126 255 80]
	...
	if (<player> = 1)
		player_number = '1'
		player_name_pos = (<cs_menu_pos> + (-180.0, -60.0))
		if ($is_network_game)
			player_name_pos = (<cs_menu_pos> + (-180.0, 0.0))
		endif
		player_title_pos = (<cs_menu_pos> + (-180.0, -45.0))
		player_title_num_pos = (<cs_menu_pos> + (-180.0, -55.0))
	else
		player_number = '2'
		player_name_pos = (<cs_menu_pos> + (655.0, -60.0))
		if ($is_network_game)
			player_name_pos = (<cs_menu_pos> + (655.0, 0.0))
		endif
		player_title_pos = (<cs_menu_pos> + (655.0, -45.0))
		player_title_num_pos = (<cs_menu_pos> + (655.0, -55.0))
	endif
	<icon_id> :GetTags
	...
	if ($current_num_players = 1)
		...
		<cs_name_text> :SetProps scale = 1.2
		GetScreenElementDims id = <cs_name_text>
		if (<width> > 256)
			fit_text_in_rectangle id = <cs_name_text> dims = ((300.0, 0.0) + (<Height> * (0.0, 1.2))) pos = (508.0, 200.0)
		endif
		...
		if ($is_network_game)
			...
			displayText {
				...
				scale = (1.333, 1.6669999)
				rgba = <purpley>
				z_priority = 3
			}
			if (<player> = 2)
				GetScreenElementDims id = <id>
				SetScreenElementProps id = <player_text_id> pos = (<player_title_num_pos> - <width> * (1.0, 0.0) - (10.0, -10.0))
			endif
		endif
	endif
	CreateScreenElement {
	...
	SpawnScriptLater \{check_player_disconnect}
	if GotParam \{blah}
