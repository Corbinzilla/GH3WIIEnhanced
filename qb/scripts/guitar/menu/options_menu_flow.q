options_select_option_fs = {
	...
	actions = [
		...
		{
			action = select_debugmenu
			flow_state = options_debugmenu_fs
			transition_right
		}
		{
			action = select_modmenu
			flow_state = options_modmenu_fs
			transition_right
		}
		{
			action = go_back
...
options_video_settings_fs = {
	...
	actions = [
		...
		{
			action = select_progressive_scan
			flow_state = ps2_480p_warning_fs
			transition_right
		}
		{
			action = go_back
	...
	actions = [
		{
			...
			func = memcard_set_options_as_successor
			flow_state_func = memcard_overwrite_warning_ingame
		}
		{
			action = select_load_game
			func = memcard_set_options_as_successor
			flow_state_func = memcard_load_lose_progress_ingame
		}
		{
			action = go_back
			flow_state = options_select_option_fs
...
options_cheats_fs = {
 ...
 options_debugmenu_fs = {
	...
	actions = [
		...
		{
			action = changedebugmenusettings
			flow_state = options_change_mod_setting_fs
		}
		{
			action = go_back
			flow_state = options_select_option_fs
 ...
 options_modmenu_fs = {
	create = create_modmenu
	destroy = destroy_modmenu
	actions = [
		{
			action = continue
			flow_state = options_select_option_fs
