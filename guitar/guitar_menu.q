...
script notify_no_home_menu 
	NotifyHMSpawned
	if ScreenElementExists \{id = HBM_Icon}
		return
	endif
	...
	CreateScreenElement \{type = SpriteElement
		...
		z_priority = 160.0
		alpha = 0}
	doScreenElementMorph \{id = HBM_Icon
		alpha = 1
		time = 0.25}
	Wait \{1.25
		seconds}
	...
	Wait \{0.25
		seconds}
	if ScreenElementExists \{id = HBM_Icon}
		DestroyScreenElement \{id = HBM_Icon}
	endif
endscript
bunny_flame_index = 1
g_anim_flame = 1
...
script create_main_menu
	...
	change \{structurename = player2_status
		character_id = axel}
	change \{default_menu_focus_color = [0 255 255 255]}
	change \{default_menu_unfocus_color = [0 255 199 255]}
	...
	options_text_off = (<training_text_off> + (-20.0, 77.0))
	...
	CreateScreenElement {
		...
		id = downloading_mod
		parent = main_menu_text_container
		text = 'Thank You For Downloading And Playing My Mod!'
		font = text_a10
		pos = (0.0, 600.0)
		rgba = [255 165 0 255]
		just = [center center]
	...
	LoginMostRecentProfile
	spawnscriptnow \{move_downloading_text}
endscript
...
script guitar_menu_highlighter \{zPri = 50}
	if GotParam \{text_id}
		...
		hilite_dims = (<width> * (1.0, 0.0) + <Height> * (0.0, 0.7) + (26.0, 6.0))
		...
		SetScreenElementProps {
			id = <be1ID>
			pos = (<hilite_pos> - <bookend_dims>.(1.0, 0.0) * (0.6, 0.0) + <Height> * (0.0, 0.15))
			dims = <bookend_dims>
			z_priority = <zPri>
		}
		SetScreenElementProps {
			id = <be2ID>
			pos = (<hilite_pos> + (<hilite_dims>.(1.0, 0.0) * (1.0, 0.0)) + <Height> * (0.0, 0.15) - (<bookend_dims>.(1.0, 0.0) * (0.1, 0.0)))
			dims = <bookend_dims>
			z_priority = <zPri>
			flip_h
		}
...
script destroy_main_menu 
	if ScreenElementExists \{id = downloading_mod}
		DestroyScreenElement id = downloading_mod
	endif
	KillSpawnedScript \{name = move_downloading_text}
...
script main_menu_select_xbox_live 
	if IsNgc
		if ($main_menu_to_nintendo_wfc_render_stall = 1)
			SkipRenderFrames \{10}
			change \{main_menu_to_nintendo_wfc_render_stall = 0}
		endif
	endif
...
endscript
default_menu_focus_color = [0 255 255 255]
default_menu_unfocus_color = [0 255 199 255]
menu_focus_color = [0 255 255 255]
menu_unfocus_color = [0 255 199 255]
