guitar_hero_cheats = [
	...
	{
		name = LargeGems
		name_text = $string_ps2_LARGE_GEMS
		var = Cheat_LargeGems
		unlock_pattern = [
			65536
			4096
...
script create_cheats_menu
	...
	CreateScreenElement {
		...
		event_handlers = [
			{focus cheats_morph_hilite params = {pos = (349.0, 377.0) dims = <dims> id = Cheat_LargeGems_Text}}
	...
	if ($<cheat> > 0)
		if ($<cheat> = 1)
			...
			if ($Cheat_LargeGems = 2)
				...
				change \{gem_start_scale1 = 0.4}
				change \{gem_end_scale1 = 0.6}
				change \{gem_start_scale2 = 0.4}
				change \{gem_end_scale2 = 0.6}
				change \{whammy_top_width1 = 8}
				change \{whammy_top_width2 = 8}
