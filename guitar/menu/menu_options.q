...
script create_options_menu 
	...
	<id> :SetTags bg_pos = (482.0, 557.0) bg_dims = (188.0, 58.0) 
	GetScreenElementDims id = <id>
	if (<width> > 195) 
		SetScreenElementProps id = <id> scale = 1
		fit_text_in_rectangle id = <id> dims = (180.0, 60.0) pos = (495.0, 566.0) 
	endif
	...
	CreateScreenElement {
		...
		event_handlers = [
			{focus options_menu_hilight}
			{unfocus retail_menu_unfocus}
			{pad_choose ui_flow_manager_respond_to_action params = {action = select_debugmenu}}
			{pad_up options_change_index params = {move = up}}
			{pad_down options_change_index params = {move = down}}
			{pad_back menu_flow_go_back}
		]
	...
	CreateScreenElement {
		...
		event_handlers = [
			{focus options_menu_hilight}
			{unfocus retail_menu_unfocus}
			{pad_choose ui_flow_manager_respond_to_action params = {action = select_modmenu}}
			{pad_up options_change_index params = {move = up}}
			{pad_down options_change_index params = {move = down}}
			{pad_back menu_flow_go_back}
		]
	}
	<id> :SetTags bg_pos = (488.0, 557.0) bg_dims = (188.0, 58.0)
	GetScreenElementDims id = <id>
	if (<width> > 180)
		SetScreenElementProps id = <id> scale = 1
		fit_text_in_rectangle id = <id> dims = (170.0, 28.0) pos = (497.0, 383.0)
