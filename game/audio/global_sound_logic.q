...
PreEncore_Crowd_Build_Unique_id = null
...
endscript
Star_Power_On_Wii = {
	effect = $Star_Power_On_Wii_Lowpass
	effect2 = $Star_Power_On_Wii_Highpass
	effect3 = $Star_Power_On_Wii_Echo
	effect4 = $Star_Power_On_Wii_Flange
}
Star_Power_On_Wii_Lowpass = {
	effect = LowPass
	name = LowPasstest1
	cutoff = 5000.0
	resonance = 10.0
}
Star_Power_On_Wii_Highpass = {
	effect = HighPass
	name = HighPasstest1
	cutoff = 300.0
	resonance = 3.0
}
Star_Power_On_Wii_Echo = {
	effect = echo
	name = InGameEcho1
	delay = 150
	DecayRatio = 0.4
...
script GH_Star_Power_Verb_On 
	...
	if (IsNgc)
		setsoundbusseffects \{$Star_Power_On_Wii
			time = 0.1}
	else
		if (<beat_duration> > 400)
			beat_duration = (<beat_duration> / 2)
...
script GH_Star_Power_Verb_Off 
	...
	HackSetReverbPlease \{stream_id = $reverb_unique_id
		enabled = false}
	...
	if (IsNgc)
		setsoundbusseffects \{$Player1EffectsWii
			time = 0.5}
	else
		setsoundbusseffects \{effect = {
				effect = sfxreverb
...
script PreloadRandomMenuSong 
	begin
	GetRandomArrayElement ($Menu_Music_Array)
	if ($prev_menu_index != <index>)
		change prev_menu_index = <index>
		break
	endif
	repeat
	PreloadSoundStream filename = (<element>.stream) samplerate = 22050 stereo = true menumusic
	return <...>
endscript
isMenuMusicOn = 0
...
script menu_music_on \{setflag = 0}
	...
	if isps2
		...
		if NOT EnableMenuMusic
			EnableMenuMusic \{disable}
	...
	if ((isps2) || (IsNgc))
		if ((IsNgc))
			if NOT ($menu_music_id = null)
				Menu_music_Checking
			endif
		endif
		PreloadRandomMenuSong
		change menu_music_id = <unique_id>
		begin
		if PreLoadStreamDone \{$menu_music_id}
			break
		endif
		Wait \{6
			gameframe}
		repeat
		StartPreloadedStream \{$menu_music_id}
		Wait \{3
			seconds}
		Menu_music_Checking
		StopSoundStream \{unique_id = $menu_music_id}
		Wait \{1
			second}
	else
...
script Menu_music_Checking 
	begin
	if (isps2)
		if IsSoundStreamPlaying \{$menu_music_id}
			if NOT LoadMoreMenuMusic
				PreloadRandomMenuSong
			endif
		elseif MenuMusicPreloaded
			StartPreloadedStream \{$menu_music_id}
		else
			break
		endif
		...
		if (IsNgc)
			if NOT IsSoundStreamPlaying \{$menu_music_id}
				break
			endif
		else
...
script Menu_Music_Off \{setflag = 0}
	...
	if ((IsNgc))
		change \{menu_music_id = null}
	endif
endscript
Intro_unique_id = null

script PlayEncoreStreamSFX 
endscript

script StopEncoreStreamSFX 

	if ((isps2) || (IsNgc))
		if NOT ($Intro_unique_id = null)
			StopSoundStream \{unique_id = $Intro_unique_id}
			change \{Intro_unique_id = null}
			WaitOneGameFrame
			WaitOneGameFrame
		endif
	endif
...
script You_Rock_Waiting_Crowd_SFX 
	...
	GetPakManCurrent \{map = zones}
	if (<pak> = z_hell)
		SoundEvent \{event = Hell_Crowd_Fast_Surge_Cheer}
	else
		SoundEvent \{event = Crowd_Fast_Surge_Cheer}
	endif
endscript
...
script GH_SFX_Intro_WarmUp 
	if ((isps2) || (IsNgc))
		StartPreloadedStream \{$Intro_unique_id}
		return
	else
		GetPakManCurrent \{map = zones}
		switch <pak>
...
script PreEncore_Crowd_Build_SFX 
	...
	switch <pak>
		case z_party
		encore_filename = 'OTHER\\ENCBLD\\B_PARTY'
		case z_dive
		encore_filename = 'OTHER\\ENCBLD\\B_DIVE'
		case z_artdeco
		encore_filename = 'OTHER\\ENCBLD\\B_DECO'
		case z_video
		encore_filename = 'OTHER\\ENCBLD\\B_VIDEO'
		case z_prison
		encore_filename = 'OTHER\\ENCBLD\\B_PRISON'
		case z_budokan
		encore_filename = 'OTHER\\ENCBLD\\B_KAN'
		case z_wikker
		encore_filename = 'OTHER\\ENCBLD\\B_WIKKER'
		case z_hell
		encore_filename = 'OTHER\\ENCBLD\\B_HELL'
		case z_soundcheck
		default
		encore_filename = 'OTHER\\ENCBLD\\B_PARTY'
	endswitch
	PreloadSoundStream filename = <encore_filename> samplerate = 33075 looping = true
	change PreEncore_Crowd_Build_Unique_id = <unique_id>
	waitforpreload_stream \{stream = PreEncore_Crowd_Build_Unique_id}
	StartPreloadedStream \{$PreEncore_Crowd_Build_Unique_id
		num_loops = -1}
endscript

script PreEncore_Crowd_Build_SFX_STOP 
	if NOT ($PreEncore_Crowd_Build_Unique_id = null)
		StopSoundStream \{unique_id = $PreEncore_Crowd_Build_Unique_id}
		change \{PreEncore_Crowd_Build_Unique_id = null}
	endif
endscript
...
script GH_SFX_Preload_Boss_Intro_Audio 
	...
	switch <pak>
		...
		if ($current_song = bosstom)
			...
			if PreloadSoundStream \{filename = 'OTHER\\INTRO\\TOM'
					samplerate = 33075
					stereo = true}
		...
		if ($current_song = bossslash)
			...
			if PreloadSoundStream \{filename = 'OTHER\\INTRO\\SLASH'
					samplerate = 33075
					stereo = true}
		...
		if ($current_song = bossdevil)
			...
			if PreloadSoundStream \{filename = 'OTHER\\INTRO\\LOU'
					samplerate = 33075
					stereo = true}
