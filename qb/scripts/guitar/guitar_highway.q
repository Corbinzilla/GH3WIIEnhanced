...
script setup_highway \{player = 1}
	...
	if ($game_mode = tutorial)
		<highway_material> = Highway_Tutorial
	endif
	...
	<starpower_scale> = (0.5 * <starpower_scale>)
	...
	SpawnScriptLater move_highway_2d_off params = {<...>}
...
script move_highway_2d_off 
	change \{start_2d_move_off = 0}
	begin
	if ($start_2d_move_off = 1)
		break
	endif
	WaitOneGameFrame
	repeat
...
script move_highway_camera_to_default_off \{player = 1}
	change \{start_2d_move_off = 1}
endscript
...
Highway_Fader_Params_2p_ps2 = {
	...
	rt_size = (640.0, 448.0)
	rt_offset = (0.0, 0.0)
	clip_dims = (320.0, 200.0)
	clip_offset = (0.0, 0.0)
	mask_dims = (640.0, 200.0)
}
...
script setup_bg_viewport 
	...
	elseif IsNgc
		...
		ScriptAssert \{'Unrecognized platform for setup_bg_viewport'}
	endif
	pos = (2000.0, 300.0)
	pos = (<pos> + <clip_offset>)
	...
	if NOT ScreenElementExists \{id = ui_clip_root}
		...
		GetPlatform
		if NOT (<platform> = Ps2 || <platform> = ngc)
