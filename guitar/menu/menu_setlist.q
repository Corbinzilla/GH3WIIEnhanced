...
script destroy_setlist_songpreview_monitor 
	...
	if (isps2)
		StopSoundStream \{unique_id = $preview_music_id}
		change \{preview_music_loading = 0}
	endif
	KillSpawnedScript \{name = setlist_songpreview_monitor}
...
endscript
preview_music_id = 0
preview_music_loading = 0

script setlist_songpreview_monitor 
	Menu_Music_Off \{setflag = 1}
	change \{setlist_unload_preview = 0}
	begin
	if ($setlist_preview_ready = 1)
		if ($setlist_unload_preview = 1)
			if isps2
				StopSoundStream \{unique_id = $preview_music_id}
				change \{preview_music_id = 0}
				change \{preview_music_loading = 0}
			else
				SongUnLoadFSB
			endif
			...
			if NOT ($current_setlist_songpreview = $target_setlist_songpreview)
				...
				song = ($target_setlist_songpreview)
				if isps2
					change \{preview_music_loading = 1}
					StopSoundStream \{unique_id = $preview_music_id}
					if (PreLoadChecksumStream <song> samplerate = 33075 stereo = true vol = $Preview_Master_Volume)
						change preview_music_id = <unique_id>
						begin
						if PreLoadStreamDone \{$preview_music_id}
							break
						endif
						Wait \{1
							gameframe}
						repeat
						StartPreloadedStream \{$preview_music_id}
						change \{preview_music_loading = 0}
					else
						change \{preview_music_id = 0}
					endif
					...
					SongUnLoadFSBIfDownloaded
					if NOT SongLoadFSB song_prefix = <song_prefix> disk_stream = true
					...
					if (IsNgc)
						PreloadStream <song_preview> buss = Music_Setlist disk_stream = true
						begin
						...
						Wait \{1
							gameframe}
						repeat
						StartPreloadedStream <unique_id> startpaused = 0 buss = Music_Setlist num_loops = -1
					else
				...
				if ((isps2) && ($preview_music_id != 0))
					if NOT IsSoundStreamPlaying \{$preview_music_id}
						change \{current_setlist_songpreview = none}
					endif
					Wait \{1
						second}
...
script change_tab \{tab = tab_setlist
	...
	if ($setlist_selection_found = 1)
		...
		change \{setlist_unload_preview = 1}
	endif
	KillSpawnedScript \{name = set_song_icon}
	spawnscriptnow \{set_song_icon
		params = {
