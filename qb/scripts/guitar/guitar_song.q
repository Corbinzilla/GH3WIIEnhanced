...
script preload_song \{StartTime = 0
	...
	if StructureContains Structure = <song_struct> name = band_playback_volume
		SetSoundBussParams {Band_Balance = {vol = ((<song_struct>.band_playback_volume) + <volume_mod>)}} time = <fadeintime>
	else
		SetSoundBussParams {Band_Balance = {vol = <volume_mod>}} time = <fadeinttime>
	endif
